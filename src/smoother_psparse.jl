...
function gs!(x::PVector, A::PSparseMatrix, b::PVector, maxiter::Int) #, start, step, stop)
    ...
    map(ranks) do rank
        gauss_seidel!(own_values(x)[rank], own_values(M_l1gs)[rank], own_values(b)[rank],maxiter=maxiter)   
    end
    consistent!(x) |> wait
    x
end
...
function extract_l1_gs_preconditioner(A::PSparseMatrix)
    ...
    IJV = map(ranks) do rank
        ...
        col = minimum(own_to_global(A.col_partition[rank]))-1
        I,J,V = Int[], Int[], Float64[]
        B_k = sparse(own_values(A)[rank])
        D_l1 = calculate_D(A.matrix_partition[rank])
        B_k .= B_k .+ D_l1
        is, js, vs = findnz(B_k)
...
struct CoarseGaussSeidel{S} <: Smoother
    sweep::S
    iter::Int
end

CoarseGaussSeidel(; iter = 1) = CoarseGaussSeidel(SymmetricSweep(), iter)
CoarseGaussSeidel(f::ForwardSweep) = CoarseGaussSeidel(f, 1)
CoarseGaussSeidel(b::BackwardSweep) = CoarseGaussSeidel(b, 1)
CoarseGaussSeidel(s::SymmetricSweep) = CoarseGaussSeidel(s, 1)

function (s::CoarseGaussSeidel)(A,x,b) where {S<Sweep}
    if S === ForwardSweep || S === SymmetricSweep || S === BackwardSweep
        gauss_seidel!(x, b,maxiter=s.iter)  
    end
    x 
end